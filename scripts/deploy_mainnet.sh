...
require_mainnet_inputs() {
  ...
  if [[ -z "$RPC_URL" ]]; then
    echo "ERROR: set STELLAR_RPC_URL to a mainnet Soroban RPC endpoint." >&2
    exit 1
  fi
  ...
  if [[ -z "$ADMIN_ADDRESS" ]]; then
    if "$CLI" keys address "$SOURCE_ACCOUNT" >/dev/null 2>&1; then
      ...
      echo "ERROR: set ADMIN_ADDRESS when STELLAR_ACCOUNT is not a local Stellar CLI identity." >&2
      exit 1
    fi
  fi
}
